...
enum SkyIcons : String {
    ...
    static let allValues = [clearDay, clearNight, rain, sleet, wind, fog, cloudy, partlyCloudyDay, partlyCloudyNight, snow]
}
...
class SkyIconVIew: UIView {
    ...
    open func setupGlyph(onView iconView: UIView, iconTypeString: String, withColor color: UIColor) {
        ...
        debugPrint(iconTypeString)
        for iconType in SkyIcons.allValues {
            debugPrint(iconType.iconType)
            if iconTypeString == iconType.iconType {
                weather.setType = iconType
            }
        }
